/*!
 * - AQUI VOCÊ PODE PERSONALIZAR O MAPA.
 * 
 *  dc.leaflet 0.3.3
    ...
    function _dc_leaflet(dc) {
        ...
        dc_leaflet.leafletBase = function (_chart) {
            ...
            var _tiles = function (map) {
                /* camada standard = mapa OSM   com todos os detalhes 
                 L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                 attribution: '&copy; <a href="https://openstreetmap.org/copyright">OpenStreetMap</a> contributors'
                 }).addTo(map);
                 */
                /*--- camada com os labels dos locais  e mapa limpo sem muitos detalhes*/
                map.createPane('labels');
                map.getPane('labels').style.zIndex = 650;
                map.getPane('labels').style.pointerEvents = 'none';
                var positronLabels = L.tileLayer('http://{s}.basemaps.cartocdn.com/light_only_labels/{z}/{x}/{y}.png', {
                    attribution: '©OpenStreetMap, ©Cinbesa',
                    pane: 'labels'
                }).addTo(map);

                /*--------------- pontos das UPAs ----------------------*/

                L.marker([-1.415, -48.466])
                        .addTo(map)
                        .bindTooltip('UPA Dasac',
                                {permanent: true, interactive: true}).bindPopup('Sede da Upa-Dasac');

                L.circleMarker([-1.415218, -48.466204],
                        {color: 'Tomato', radius: 10})
                        .addTo(map)
                        // .bindTooltip('Me too', {direction: 'center'}) 
                        ;

                L.marker([-1.315865, -48.456106])
                        .addTo(map)
                        .bindTooltip('UPA Daico',
                                {permanent: true, interactive: true, direction: 'right'}).bindPopup('Sede da Upa-Daico');

                L.circleMarker([-1.315865, -48.456106],
                        {color: 'Tomato', radius: 10})
                        .addTo(map)
                        //   .bindTooltip('Me too', {direction: 'center'}) 
                        ;
                
                L.marker([-1.471183, -48.450538])
                        .addTo(map)
                        .bindTooltip('UPA TF',
                                {permanent: true, interactive: true, direction: 'right'}).bindPopup('Sede da Upa-TF');

                L.circleMarker([-1.471183, -48.450538],
                        {color: 'Tomato', radius: 10})
                        .addTo(map)
                        //   .bindTooltip('Me too', {direction: 'center'}) 
                        ;
            ...
            dc.override(_chart, 'on', function (event, callback) {
                ...
                if (leaflet_events.indexOf(event) >= 0) {
                    ...
                    return _chart._on(event, callback);
            });
...
//Legend code adapted from http://leafletjs.com/examples/choropleth.html
        ...
        dc_leaflet.choroplethChart = function (parent, chartGroup) {
            ...
            var _featureStyle = function (feature) {
                ...
                if (v && v.d) {
                    ...
                    if (_chart.filters().indexOf(v.d.key) !== -1) {
                        ...
                        options.color = "RED";   // a cor que preenche (fillColor) os limites do filtro
                    }
                }
                return options;
            };
            ...
            dc.override(_chart, '_doRedraw', function () {
              //  console.log("_geojsonLayer.clearLayers();"); 
            ...
            var processFeatures = function (feature, layer) {
                /*------------ adiciona popup do nome do bairro  -----------*/
                //Define mouse events   
